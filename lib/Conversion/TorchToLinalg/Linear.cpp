...
public:
  ...
  matchAndRewrite(AtenConvolutionOp op, OpAdaptor adaptor,
    ...
    Value cFalse =
        rewriter.create<arith::ConstantOp>(loc, BoolAttr::get(context, false));
    Value notTransposed = rewriter.create<arith::CmpIOp>(
        loc, arith::CmpIPredicate::eq, adaptor.transposed(), cFalse);
    rewriter.create<cf::AssertOp>(
        loc, notTransposed, rewriter.getStringAttr("unimplemented: only non-transposed convolution supported"));
